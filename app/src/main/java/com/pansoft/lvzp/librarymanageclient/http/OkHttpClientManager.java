...
public class OkHttpClientManager {
    ...
    public  void asyncGetParams(String url, Map<String, Object> params, HttpResultCallback callback) {
        url += getGetParams(params);
        asyncGet(url, callback);
    }

    public <T> void asyncGet(String url, final HttpResultCallback callback) {
        Request request
                = new Request.Builder()
    ...
    public <T> void asyncPostJson(String url, Object jsonBean, final HttpResultCallback callback) {
    ...
    public <T> void asyncPost(String url, Map<String, Object> params, final HttpResultCallback callback) {
        FormBody.Builder formBuilder = _getFormBuilder(params);
        Request request = getRequest(url, formBuilder.build());
        mOkHttpClient.newCall(request).enqueue(new SimpleCall(callback));
    }

    private class SimpleCall implements Callback {

        private HttpResultCallback mCallback;

        SimpleCall(HttpResultCallback callback) {
            mCallback = callback;
        }
    ...
    private <T> void buildResultBean(String resultJson, HttpResultCallback callback) {
